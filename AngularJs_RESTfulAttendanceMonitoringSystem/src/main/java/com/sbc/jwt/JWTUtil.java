...
public class JWTUtil implements Serializable {
	...
	public static final long JWT_REFRESH_TOKEN_EXPIRATION = 10*60;			// 5 min
	public static final long PASSWORD_RESET_TOKEN_EXPIRATION = 2*60;		// 2 min	
	...
	private String accessTokenBuilder(Map<String, Object> claims, String username) {
		...
		jwtTokenBuilder = Jwts.builder()
						...
						.compact();
		System.out.println("jwtTokenBuilder=" + jwtTokenBuilder);
	 ...
	 *    compaction of the JWT to a URL-safe string 
	 */
	private String passwordResetTokenBuilder(Map<String, Object> claims, String username) {
		String jwtTokenBuilder = null;		
		jwtTokenBuilder = Jwts.builder()
						.setClaims(claims)
						.setIssuer(TOKEN_ISSUER)
						.setSubject(username)
						.setIssuedAt(new Date(System.currentTimeMillis()))
						.setExpiration(new Date(System.currentTimeMillis() + PASSWORD_RESET_TOKEN_EXPIRATION * 1000))		// convert sec. to milliseconds
						.signWith(SIGNATURE_ALGORITHM, secret)
						.compact();
		System.out.println("jwtTokenBuilder=" + jwtTokenBuilder);
		return jwtTokenBuilder;
	...
	public String generateTokenForPasswordReset(UserDetails userDetails) {
		...
		Claims claims = Jwts.claims().setSubject(userDetails.getUsername());
        claims.put("scopes", userDetails.getAuthorities().stream().map(s -> s.toString()).collect(Collectors.toList()));
		String access_token =  passwordResetTokenBuilder(claims, userDetails.getUsername());		
		return access_token;
	}
